...
func processGlobalListDistributions(tableConfig *utilities.TableConfig, account *utilities.ExtensionConfigurationAwsAccount) ([]map[string]string, error) {
	resultMap := make([]map[string]string, 0)
...
func processAccountListDistributions(account *utilities.ExtensionConfigurationAwsAccount) ([]map[string]string, error) {
	...
	result, err := processGlobalListDistributions(tableConfig, account)
	if err != nil {
		return resultMap, err
	}
	resultMap = append(resultMap, result...)
